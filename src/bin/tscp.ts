...
async function command() {
	...
	const projects = cli_options.build
		? get_ts_projects_paths(config)
		: [get_ts_project_paths(config)];
	...
	if (cli_options.watch) {
		return watch_files(projects, config)
			...
			.catch((e) => {
				console.error(
					'Something went wrong during watching the non-typescript files',
				);
				throw e;
			});
	}
			...
			if (projects) {
				...
				console.log(
					projects
						.map(
							({ project_name }) =>
								`${color_log(project_name, console_colors.FgGreen)}`,
						)
						.join('\n'),
				);
			} else {
