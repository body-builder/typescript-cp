 ...
 * @param config
 */
export async function collect_projects_files(
	projects: TsProject[],
	config: Config,
	...
	return Promise.all(
		projects.map(async (project): Promise<TsProjectWithFiles> => {
 ...
 */
export async function collect_projects_files_flat(
	globed_projects: TsProjectWithFiles[],
): Promise<{ source_path: string; target_path: string }[]> {
	const all_projects_files = await Promise.all(
		globed_projects.map(async (project) => {
...
export async function copy_files(
	...
	await Promise.all(
		all_projects_files.map(async ({ target_path }) => {
	...
	// Link files
	await Promise.all(
		all_projects_files.map(async ({ source_path, target_path }) => {
...
export async function watch_files(
	...
	const ignore_glob = `{${get_ignore_list(config, projects)
		.map((rule) => {
			if (!rule.startsWith('**/')) {
				return `**/${rule}`;
			}
	 ...
	 * @param source_path
	 */
	function get_target_path(source_path: string): {
		project_name: string;
		filename: string;
		source_path: string;
		target_path: string;
	} {
		const parent_project = globed_projects.find(({ base_path }) =>
			source_path.startsWith(base_path),
		);

		if (!parent_project) {
			throw new Error(
				`Could not find the TS project to which the file belongs to: '${source_path}'`,
			);
		}
	...
	async function watch_idle_log(
		...
		console.log(
			color_log(
				globed_projects.map(({ project_name }) => `${project_name}`).join('\n'),
	...
	watcher
		...
		.on('add', async (unsafe_source_path) => {
			...
			const { target_path, project_name, filename } =
				get_target_path(source_path);
			await copy_file_or_directory(unsafe_source_path, target_path, config);
			// Do not pollute the console with the bootstrapping data
			if (isReady) {
				console.log(
					color_log(`${project_name}/${filename}`, console_colors.FgGreen),
					'added',
				);
				await watch_idle_log();
			}
		...
		.on('change', async (unsafe_source_path) => {
			...
			console.log(
				color_log(`${project_name}/${filename}`, console_colors.FgYellow),
				'changed',
			);
			await watch_idle_log();
		})
		.on('unlink', async (unsafe_source_path) => {
			...
			console.log(
				color_log(`${project_name}/${filename}`, console_colors.FgRed),
				'deleted',
			);
			await watch_idle_log();
		});
